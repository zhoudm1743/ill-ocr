...
基于 **FastAPI + RapidOCR (ONNX Runtime)** 的 HTTP OCR 服务，支持通用文字识别、**身份证正反面结构化提取**与**营业执照结构化提取**。

> 生产环境默认部署路径：`/opt/paddle-ocr/`  
> 默认端口：`8000`
...
```
/opt/paddle-ocr/
├── app/                    # Python 源码（volume 挂载到容器 /app）
│   ├── main.py             # FastAPI 入口，路由定义
│   ├── response.py         # 统一响应封装 success / fail
│   ├── idcard_parser.py    # 身份证结构化解析逻辑
│   └── biz_license_parser.py # 营业执照结构化解析逻辑
...
将以下文件上传到服务器 `/opt/paddle-ocr/`：

```bash
mkdir -p /opt/paddle-ocr/app /opt/paddle-ocr/logs
# 上传：app/main.py app/response.py app/idcard_parser.py app/biz_license_parser.py
...
# 营业执照
curl -s -X POST http://localhost:8000/ocr/biz-license -F "file=@biz_license.jpg"
```
...
`app/` 目录已通过 volume 挂载到容器 `/app`，因此修改宿主机文件后只需重启容器即可生效：

```bash
# 上传新文件到 /opt/paddle-ocr/app/
# 然后重启容器
...
```bash
cd /opt/paddle-ocr/app   # 或你的本地项目目录
...
请求 → FastAPI 路由 (main.py)
     ...
     营业执照 OCR：biz_license_parser.py 结构化提取字段
```
...
添加新证件类型时，建议新建独立 parser 模块（如 `biz_license_parser.py`），保持模块独立。
...
mock_items = [
    {"text": "名称", "confidence": 0.99, "box": [[0,0],[0,0],[0,0],[0,0]]},
    {"text": "北京某某科技有限公司", "confidence": 0.98, "box": [[0,0],[0,0],[0,0],[0,0]]},
    {"text": "统一社会信用代码91110000MA01234567", "confidence": 0.99, "box": [[0,0],[0,0],[0,0],[0,0]]},
]
print(parse_biz_license(mock_items))
```
...
```
POST /ocr/biz-license?include_raw=false
Content-Type: multipart/form-data
```
...
```json
{
  ...
  "data": {
    ...
    "fields": {
      ...
      "establishment_date": "2020年01月15日",
      "establishment_date_iso": "2020-01-15",
      "business_term": "2020年01月15日至长期",
...
```
POST /ocr/biz-license/base64?include_raw=false
Content-Type: application/json
```
...
| 1.3.0 | 新增营业执照 OCR 接口 `/ocr/biz-license` |
